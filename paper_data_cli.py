#!/usr/bin/env python3
"""
CLI tool for managing the bibtex-to-PDF mapping worklist.
"""
...
class WorklistManager:
    ...
    def set_extracted_paper_info(self, bib_key: str, info_data: dict) -> bool:
        """Set the extracted_paper_info field for a bibtex entry."""
        if bib_key in self.data['bibtex']:
            self.data['bibtex'][bib_key]['extracted_paper_info'] = info_data
            self.save()
            return True
        return False

    def set_raw_bibtex(self, bib_key: str, raw_bibtex: str) -> bool:
        """Set the raw_bibtex field for a bibtex entry."""
        if bib_key in self.data['bibtex']:
            self.data['bibtex'][bib_key]['raw_bibtex'] = raw_bibtex
            self.save()
            return True
        return False

    def set_under_submission(self, bib_key: str, under_submission: bool) -> bool:
        """Set the under_submission field for a bibtex entry."""
        if bib_key in self.data['bibtex']:
            self.data['bibtex'][bib_key]['under_submission'] = under_submission
            self.save()
            return True
        return False
    ...
    def get_bibtex_by_pdf_stem(self, pdf_stem: str) -> Optional[str]:
        ...
        for pdf_name, pdf_entry in self.data['pdfs'].items():
            ...
            if Path(pdf_name).stem == pdf_stem:
                # Return the mapped bibtex key if it exists
                return pdf_entry.get('mapped_bibtex')
        return None

    def get_bibtex_with_markdown(self, status: Optional[str] = 'MAPPED') -> List[Tuple[str, Dict]]:
        """Get bibtex entries that have an associated PDF with markdown."""
        results = []
        for bib_key, bib_entry in self.data['bibtex'].items():
            # Filter by status if specified
            if status and bib_entry['status'] != status:
                continue

            # Check if this bibtex has a mapped PDF
    ...
    def create_mapping(self, bib_key: str, pdf_file: str, confidence: str = "high", notes: str = None, found: bool = False):
        """Create a mapping between bibtex and PDF."""
        # Update bibtex entry
        if bib_key in self.data['bibtex']:
            self.data['bibtex'][bib_key]['status'] = 'MAPPED'
            self.data['bibtex'][bib_key]['mapped_pdf'] = pdf_file
            if notes:
                self.data['bibtex'][bib_key]['notes'] = notes
            if found:
                self.data['bibtex'][bib_key]['found_pdf'] = True
                self.data['bibtex'][bib_key]['found_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # Update PDF entry
        if pdf_file in self.data['pdfs']:
            self.data['pdfs'][pdf_file]['status'] = 'MAPPED'
            self.data['pdfs'][pdf_file]['mapped_bibtex'] = bib_key
        else:
            # Create new PDF entry for newly found PDFs
            self.data['pdfs'][pdf_file] = {
                'status': 'MAPPED',
                'mapped_bibtex': bib_key,
                'has_text': False,
                'newly_found': True,
    ...
    def remove_mapping(self, bib_key: str):
        ...
        if mapping_to_remove:
            ...
            # Update bibtex entry
            if bib_key in self.data['bibtex']:
                self.data['bibtex'][bib_key]['status'] = 'NOT_INVESTIGATED'
                self.data['bibtex'][bib_key]['mapped_pdf'] = None

            # Update PDF entry
            if pdf_file in self.data['pdfs']:
                self.data['pdfs'][pdf_file]['status'] = 'NOT_INVESTIGATED'
                self.data['pdfs'][pdf_file]['mapped_bibtex'] = None
    ...
    def delete_entry(self, item_type: str, key: str) -> bool:
        ...
        # If it's a bibtex entry, remove any mapping first
        if item_type == 'bibtex':
    ...
    def get_stats(self) -> Dict:
        ...
        stats = {
            'total_bibtex': len(self.data['bibtex']),
        ...
        # Count by status
        for entry in self.data['bibtex'].values():
            status = entry['status']
            stats['bibtex_status'][status] = stats['bibtex_status'].get(status, 0) + 1
    ...
    def get_unmapped(self) -> Dict[str, List[str]]:
        ...
        for key, entry in self.data['bibtex'].items():
            if entry['status'] != 'MAPPED':
                unmapped['bibtex'].append(key)
    ...
    def suggest_matches_for_pdf(self, pdf_name: str, max_suggestions: int = 5) -> List[Tuple[str, Dict, float]]:
        """Suggest potential bibtex matches for a PDF."""
...
@click.pass_context
def cli(ctx):
    """Worklist management tool for bibtex-to-PDF mapping."""
    ctx.ensure_object(dict)
    ctx.obj['manager'] = WorklistManager()

@cli.group()
def bibtex():
    """Bibtex entry management commands."""
    pass

@bibtex.command('set-extracted-info')
@click.argument('bib_key')
@click.argument('json_file_path')
@click.pass_context
def bibtex_set_extracted_info(ctx, bib_key, json_file_path):
    """Set the extracted_paper_info field for a bibtex entry from a JSON file."""
    ...
    if manager.set_extracted_paper_info(bib_key, info_data):
        ...
        click.echo(f"Error: Bibtex entry {bib_key} not found", err=True)
        sys.exit(1)

@bibtex.command('list-with-markdown')
@click.option('--status', default='MAPPED', help='Filter by bibtex status (default: MAPPED)')
@click.pass_context
def bibtex_list_with_markdown(ctx, status):
    """List bibtex entries that have associated markdown files."""
    manager = ctx.obj['manager']
    results = manager.get_bibtex_with_markdown(status)
...
@bibtex.command('get-by-pdf-stem')
@click.argument('pdf_stem')
@click.pass_context
def bibtex_get_by_pdf_stem(ctx, pdf_stem):
    """Find bibtex key by PDF filename stem (without extension)."""
    manager = ctx.obj['manager']
    bib_key = manager.get_bibtex_by_pdf_stem(pdf_stem)
    if bib_key:
        click.echo(bib_key)
    else:
        click.echo(f"Error: No bibtex entry found for PDF stem: {pdf_stem}", err=True)
        sys.exit(1)

@bibtex.command('set-raw-bibtex')
@click.argument('bib_key')
@click.argument('bibtex_file')
@click.pass_context
def bibtex_set_raw_bibtex(ctx, bib_key, bibtex_file):
    """Set the raw_bibtex field for a bibtex entry from a file."""
    manager = ctx.obj['manager']
...
@bibtex.command('set-under-submission')
@click.argument('bib_key')
@click.argument('value', type=click.Choice(['true', 'false']))
@click.pass_context
def bibtex_set_under_submission(ctx, bib_key, value):
    """Set the under_submission field for a bibtex entry."""
    manager = ctx.obj['manager']
...
@tags.command('add')
@click.argument('item_type', type=click.Choice(['bibtex', 'pdfs']))
@click.argument('key')
...
def search(ctx, item_type, field, query):
    ...
    if results:
        ...
        for key, entry in results[:10]:
            click.echo(f"  {key}")
            if item_type == 'bibtex' and 'title' in entry:
...
def stats(ctx):
    ...
    click.echo("\n=== OVERALL STATISTICS ===")
    click.echo(f"Total bibtex entries: {stats['total_bibtex']}")
    click.echo(f"Total PDFs: {stats['total_pdfs']}")
    click.echo(f"Mapped: {stats['mapped']}")

    click.echo("\n=== BIBTEX STATUS ===")
    for status, count in stats['bibtex_status'].items():
        pct = (count / stats['total_bibtex']) * 100
...
def unmapped(ctx):
    ...
    click.echo(f"Unmapped bibtex entries: {len(unmapped['bibtex'])}")
    click.echo(f"Unmapped PDFs: {len(unmapped['pdfs'])}")

    if click.confirm("Show unmapped bibtex entries?"):
        for key in unmapped['bibtex'][:20]:
            click.echo(f"  - {key}")
        if len(unmapped['bibtex']) > 20:
            click.echo(f"  ... and {len(unmapped['bibtex']) - 20} more")
...
def pdf_investigate(ctx, pdf_file, auto_map, threshold):
    """Investigate a PDF and find potential bibtex matches."""
    manager = ctx.obj['manager']
...
def report(ctx):
    ...
    click.echo(f"\n📚 Total items:")
    click.echo(f"  - Bibtex entries: {stats['total_bibtex']}")
    click.echo(f"  - PDF files: {stats['total_pdfs']}")

    click.echo(f"\n✅ Progress:")
    click.echo(f"  - Completed mappings: {stats['mapped']}")
    click.echo(f"  - Unmapped bibtex: {len(unmapped['bibtex'])}")
    click.echo(f"  - Unmapped PDFs: {len(unmapped['pdfs'])}")

    if stats['mapped'] > 0:
        coverage = (stats['mapped'] / stats['total_bibtex']) * 100
        click.echo(f"  - Coverage: {coverage:.1f}% of bibtex entries")
    ...
    no_pdf = [k for k, v in manager.data['bibtex'].items() if v['status'] == 'NO_PDF']
    if no_pdf:
        click.echo(f"\n⚠️  Papers needing PDFs: {len(no_pdf)}")
        for key in no_pdf[:5]:
            entry = manager.data['bibtex'][key]
            click.echo(f"  - {key}")
            if entry.get('title'):
