...
const { TopNav, Footer, Icon, fetchPapers, parseTags, venueOf, fmtAuthors } = V6;
...
V6.MarkdownView = function MarkdownView({ paperId }) {
...
V6.BibtexView = function BibtexView({ bibtex }) {
...
V6.PubRow = function PubRow({ p, deep }) {
  const { MarkdownView, BibtexView } = V6;
  ...
  // Deep-linked from a chat citation: land centered, flash, open the summary.
  useEffect(() => {
    if (deep && liRef.current) {
      liRef.current.scrollIntoView({ block: 'center' });
      if (tldr) setOpen('about');
    }
  }, [deep]);
  const title = String(p.title || '').replace(/``/g, '\u201C').replace(/''/g, '\u201D').replace(/[{}]/g, '');
  const venue = String(venueOf(p) || '').replace(/\.$/, '');
  return (
    <li ref={liRef} className={deep ? 'deep' : ''}>
      <span className="hangdate">{p.year}</span>
      <span style={{ minWidth: 0, flex: 1 }}>
        <span className="t"><a href={`/papers/${V6.slugOf(p.id)}`}>{title}</a></span>{' '}
...
V6.Pubs = function Pubs() {
  ...
  // #<paper-id> in the URL (from a chat citation) selects a paper to land on.
  useEffect(() => {
    if (!papers || !window.location.hash) return;
    try {
      const id = decodeURIComponent(window.location.hash.slice(1));
      if (papers.some(p => p.id === id)) setDeepId(id);
    } catch (_) {}
  }, [papers]);
  ...
  return (
    <div className="page">
      <header>
        <h1>Publications</h1>
        <p className="positioning">The complete list: {papers ? papers.length : '…'} papers,
          none omitted. Every paper with PDF and BibTeX, and where available a readable
          full text and a one-paragraph summary.</p>
      </header>
      ...
      <section>
        ...
        {byYear.map(g => (
          <div key={g.year}>
            ...
            <ul className="pubs">
              {g.items.map(p => <PubRow key={p.id} p={p} deep={p.id === deepId} />)}
            </ul>
          </div>
        ))}
      </section>

      <p className="aside" style={{ marginTop: '2rem' }}>Citations follow the CV; the BibTeX is the authoritative form.</p>
      <Footer />
    </div>
  );
};
