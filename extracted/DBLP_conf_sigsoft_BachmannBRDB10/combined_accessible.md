...
> **Image description:** A plain-text SVN commit log showing revision r291558 by user “pquerna” on 2005-09-26 with a two-line message. The entry lists the changed path (M /httpd/httpd/trunk/Makefile.in) and the brief comment: “As recommended by nd, build docs for all languages.” There is no BugZilla ID or other bug-tracker reference in the message — the paper uses this commit as an example of a fix tied to a mailing-list discussion where the commit text gives almost no evidence it is a bug-fix, so automated linking heuristics would likely miss it.
