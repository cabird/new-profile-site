...
[Image: page6_img_table_1.png] A two-column ranked list of Linux source subdirectories showing the ten directories with the highest proportions of unique changes (left) and the ten with the highest proportions of non-unique changes (right). On the unique side, fs/jbd/ tops the list with 97.52% of its changed lines marked unique, followed by drivers/video/ (94.25%) and fs/ext4/ (93.79%). On the non-unique side, sound/drivers/ is highest at 94.34%, then lib/ (92.86%) and net/netlink/ (91.67%). These figures illustrate the paper’s finding that uniqueness is concentrated in particular modules (e.g., journaling fs is highly unique) while other areas (e.g., sound drivers) exhibit high repetition of similar changes.
