...
> **Image description:** A screenshot of the prototype’s “Agent Notes” panel that presents the agent’s current reasoning about a differential test: it states the diff is an ErrorCode change (specific codes redacted) and tentatively attributes it to a deprecation/rollout observed in historical comments. Below the reasoning are two blue chips showing UUIDs (5325ecd2-... and 6b2fe749-...) that link the agent’s claims to specific artifacts for traceability. A green “Next Steps” box lists actionable checks (e.g., call 'lookup_error' and review test/production logs) and suggests labeling the diff as 'NewFeature' if rollout patterns match; beneath that is a “Human Guidance” area with an 11s timer and a user message asking the agent to search team messages for related discussion. The view demonstrates how the agent documents belief updates, cites concrete artifact IDs, and solicits synchronous human input to steer the investigation.
