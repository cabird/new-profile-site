...
> **Image description:** A directed graph showing the mailing-list social network for highly active Postgres participants (labels are shown on each oval; roughly 30 labeled nodes). Arrows are directed reply ties (this figure uses a threshold of at least 150 messages between participants), and the layout shows a clear hub-and-spoke structure: two centrally placed oval nodes have dense incoming and outgoing edges to most other nodes, while many peripheral nodes connect primarily to one of those hubs. A small number of reciprocal links appear and one peripheral node has a self-loop, visually reinforcing the paper’s finding that a few participants occupy highly central positions in the mailing-list network.
