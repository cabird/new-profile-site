...
> **Image description:** The figure shows a two-part view of a small Java snippet (lines 5–8 from TextRunnerTest.java) and the corresponding tokenization that NATURALIZE uses for learning formatting. The original source (top) is mapped (bottom) to a stream where identifiers and literals are collapsed to ID/LIT, language keywords (e.g., public, throws) remain, and whitespace is encoded as INDENT and SPACE tokens annotated with metadata (e.g., number of spaces/tabs and newlines). This representation captures both line-level indentation (INDENT with newline/indent magnitude) and intra-line spacing (SPACE or SPACE0) so an n-gram language model can learn and suggest conventional formatting.
...
> **Image description:** A small table reporting the percentage of code review threads (column “Reviews”) and commit log messages (column “Commits”) that contained feedback about coding conventions, identifier naming, and formatting, with 95% confidence intervals shown in parentheses. Results: “Conventions” occurred in 38% of reviews (31%–46%) but only 4% of commits (3%–6%); “Naming” in 24% of reviews (17%–31%) vs 1% of commits (1%–2%); and “Formatting” in 9% of reviews (6%–15%) vs 2% of commits (1%–3%). One-sided proportion tests yield p ≪ 0.01 for each row (rightmost column), indicating the differences between review feedback and commit messages are statistically significant.
