...
![Issue topic-plots of all FLOSS developer authors of Craft.NET](page9_img_1.png)

> **Image description:** A 20-panel small-multiples figure (one panel per LDA topic) plotting average commit relevance over time for the Craft.NET project, with the x‑axis spanning roughly July 2012 to January 2013 and the y‑axis labelled “Avg. Commit Relevance.” Each panel is titled “Topic N” followed by the top-ranked LDA words (the paper’s representation of topic content), and vertical red dashed lines mark releases or milestones. Panels show varied temporal patterns: several topics (for example Topic 1, Topic 3, Topic 6, Topic 9, Topic 13, Topic 19 and Topic 20) exhibit pronounced spikes of relevance indicating concentrated commit activity, whereas other topics remain low or diffuse across the interval. The differing peak heights between panels (some panels reach values >1–2 while others stay below 0.5) indicate that the magnitude and timing of issue-related work vary substantially by topic, and some spikes appear near the red release lines suggesting clustered effort around milestones.
...
> **Image description:** A multi-panel collage of area/time-series plots showing, for many FLOSS projects, the average per-time-window relevance of commits to individual LDA-extracted issue topics. Each subplot is titled with the project and a topic number plus the top-ranked topic words (the LDA word lists), the X axis is time and the Y axis is average commit relevance (larger = more relevant). Red dashed vertical lines mark project releases or milestones; panels display varied behavior (some topics show sharp spikes near releases, others show sustained or intermittent activity) that was presented to developers during the survey to validate whether these topic-relevance trends matched their perception of work on those topics.
