...
> **Image description:** The table maps ten GOF design patterns (left column) to the specific ‘‘key’’ roles (right column) the authors use to recognize the same pattern instance across releases. Examples include Abstract Factory → {abstract factory, abstract product}; Adapter → {target, adapter, adaptee}; Command → {invoker, command, client}; Composite → {composite, component}; Decorator → {component, decorator}; Observer → {subject, observer}; Prototype → {prototype}; State/Strategy → {state/strategy, context}; Template Method → {abstract class}; Visitor → {visitor, element}. In the study these listed roles must remain bound to the same class names in two consecutive releases for a motif to be counted as a stable (duplicate) pattern instance.
...
> **Image description:** Close-up, head-and-shoulders color photograph of an adult man shown in a slightly turned frontal view. He has very short-cropped dark hair, a clean-shaven face, and wears rectangular dark-framed eyeglasses and a light-colored collared shirt; his expression is neutral. The image appears as an author portrait in the paper (page27_img_1.png) with a softly blurred indoor background and a tight left-side crop.
...
> **Image description:** Close-up color headshot of one of the paper’s authors used with the author biographies. The image shows a smiling adult male with short dark hair, light stubble, and light skin, wearing a light-colored collared shirt with faint stripes; the face is turned slightly toward the camera. The background is an indoor surface with diagonal dark elements; the photo is tightly cropped on the right and top, leaving a white margin on the left from the page layout.
