...
> **Image description:** A four-column table summarizing four AI systems requested by developers in the Development block (N=353), with columns for the system name, the problem it addresses, example capabilities, and constraints/guardrails. The rows correspond to: (1) Scoped-PR builder for tech-debt removal (50.1%) — addressing large, brittle refactors by proposing small, reviewable atomic diffs; (2) Embedded quality gate for code and tests (27.8%) — providing diff-anchored error and coverage feedback before a PR is opened; (3) Trace-to-diff root-cause workbench (19.5%) — automating evidence collection, failure timelines, and hypothesis generation for incidents; and (4) Repository context graph for cross-file changes (18.4%) — maintaining a persistent code-and-history graph to compute impact reports for proposed edits. Example capabilities emphasize deriving minimal impacted areas, attaching rationale from PR discussions/ADRs, generating succinct quality reports or patches, and producing reproducible failure timelines; recurring constraints require explicit human approval (no automatic commits or one-shot refactors), explicit uncertainty/provenance signaling, sandboxed reproduction, and flagging missing domain context rather than inventing it.
