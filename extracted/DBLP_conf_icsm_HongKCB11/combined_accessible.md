...
> **Image description:** Top row shows identified communities as circles (radius ∝ number of developers) and the bottom row lists BOF (birds‑of‑a‑feather) meetings. Directed arrows connect each detected community to BOF meetings and are annotated with overlap fractions (for example 0.70 and 1.0) that indicate the proportion of a BOF’s participants contained in that community. The rightmost community (labeled A) contains 70% of BOF X and 100% of BOF Y, illustrating that a single detected community can encompass multiple BOF meetings. The visualization demonstrates that Louvain‑detected communities largely correspond to offline BOF groups (most BOFs map predominantly into one community, with few splits).
