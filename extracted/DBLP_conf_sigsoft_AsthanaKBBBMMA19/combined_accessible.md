...
> **Image description:** A ranked bar chart comparing how often WhoDo suggested each reviewer when run without load balancing (blue bars) versus with aggressive load balancing (orange bars). The x‑axis lists reviewers sorted by decreasing suggestion frequency for the no-load‑balancing run (leftmost reviewers are the most frequently suggested), and the y‑axis is the number of suggestions (up to ~45). Without load balancing the distribution is highly skewed with a few reviewers suggested very frequently (peaks ~40+), while aggressive load balancing lowers those peaks and produces a much flatter distribution — several mid and low-ranked reviewers receive more suggestions (orange bars exceed blue) as load is redistributed. In the simulation (θ=0.5) the mean suggestion frequency drops slightly from 10.10 to 9.34 and the standard deviation falls from 10.17 to 5.67, indicating reduced imbalance at the cost of shifting suggestions toward less-frequent reviewers.
