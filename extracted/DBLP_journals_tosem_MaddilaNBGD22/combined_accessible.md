...
> **Image description:** Table of per-repository counts showing distinct files edited concurrently and non-concurrently, the number of bug-fix pull requests, and the share of files in each category. Across the six repositories there are 32,534 distinct concurrently edited files, 50,533 distinct non-concurrently edited files, and 54,127 bug-fix pull requests in total; overall 39.1% of files were seen edited concurrently and 60.9% non-concurrently. Per-repository concurrent-file share ranges from 35.2% (Repo-5) to 42.4% (Repo-4), and the number of bug-fix PRs per repo ranges from 4,781 (Repo-1) to 15,678 (Repo-2).
