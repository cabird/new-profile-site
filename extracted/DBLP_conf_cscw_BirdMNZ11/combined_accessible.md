...
> **Image description:** A small developer–module bipartite graph where circles are developers and squares are modules; numeric labels on edges are commit counts (edge weights). Module “b” (left square) is connected to Eric (5), Fu (6), Dan (4) and Bob (2); module “a” (right square) is connected to Alice (6), Bob (4), Go (2) and Hin (5). A third module “c” is partially shown below/right, connected from Go (7) and Hin (4). The figure highlights developers (Bob, Hin) who contribute to multiple modules and thus act as cross-module connectors — a topological property the paper uses (centrality and commit counts) to predict post-release faults.
...
> **Image description:** A schematic socio-technical graph showing modules (green/blue circles labeled A–G) and developers (pink rounded boxes labeled Fred and Ram). Solid directed edges between circles represent technical module dependencies (A↔B/C and many small arrows around D/E/F/G indicating other dependency connections), while dashed lines connect developers to modules to indicate contribution relationships (Fred linked to D and E and Ram linked to F and G, both also dashed-linked to central module A). The diagram emphasizes a central module (A) that is both technically connected to other modules and touched by multiple developers, illustrating how the paper combines contribution and dependency edges into one graph to improve prediction of failure-prone modules.
