...
> **Image description:** Scatter plot (Fig. 14) shows average precision on the y-axis against a maximum actor tenure threshold (years) on the x-axis; each point is the precision computed after discarding events whose change author has tenure above the threshold. Two series are plotted: grey circles for the Recency model and orange triangles for the Recency+Size model. Both models show higher precision for low-tenure authors (≈0.56 for Recency and ≈0.61 for Recency+Size near 0 years) that steadily declines as the tenure threshold increases and then stabilizes around 0.48–0.51 after about 5–6 years. The Recency+Size model maintains higher precision than Recency across all thresholds, and the authors note this pattern may reflect that less-experienced authors’ choices align better with recommendations (though causality is not established).
