...
> **Image description:** Three adjacent bar charts for the Apache project plotting number of active people (y-axis) against tenure in 30‑day epochs (x-axis). Left: non-developer mailing‑list activity shows a very large initial cohort (a peak ~2,000 people at the first epoch) that falls steeply in the first months and then declines toward zero by ~100 epochs. Middle: developer mailing‑list activity starts much lower (≈50–60 active people early on) and decays gradually over roughly 120 epochs. Right: active committers (file‑commit activity) likewise begin near ≈60 at the start of developer tenure and decrease steadily to single digits by about 50–70 epochs. Together the panels show that mailing‑list participation and committing both decline with tenure, with the non‑developer pool shrinking far more rapidly than the smaller, longer‑lived developer/committer core.
...
> **Image description:** Smoothed hazard function (Postgres) plotting the instantaneous rate at which mailing‑list participants become developers (y‑axis) against tenure in years (x‑axis, 0–5). The curve rises from near 0.001 at entry to a peak of about 0.002–0.0025 around year 1, then falls steadily, reaching roughly 0.0005 by year 5. A small secondary bump appears near year 3 before the continued decline. This non‑monotonic shape (early rise, peak ~1 year, then decline) visually supports the paper’s finding that immigration rates in Postgres increase early in tenure and then decrease over longer tenures.
