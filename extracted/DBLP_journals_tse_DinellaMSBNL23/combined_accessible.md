...
> **Image description:** A four-column table showing Base (O: base.js), Variant A (a.js), Variant B (b.js), and the merged Resolution (m.js) for two numbered examples. In example (1) the base contains “y = 42;”, A adds “x = 1;” above that and B adds “z = 43;” below; the unstructured diff3 merge produces a combined resolution with x = 1; y = 42; z = 43; (both edits preserved). In example (2) A inserts “x = 1;” before the unchanged y line while B inserts “z = 43;” before the same y line; because these independent edits fall in the same diff3 slot, diff3 reports a CONFLICT even though the changes do not semantically interfere.
...
> **Image description:** This table reports DEEPMERGE’s top-1 synthesis accuracy (threshold = 0, i.e., no confidence filtering) broken down by total number of lines in the conflicting region. For merges of 1–3 lines accuracy is 78.40%, for 4–5 lines 56.50%, for 6–7 lines 37.04%, for 8–10 lines 10.87%, and for merges with more than 10 lines 2.93%. The numbers show a steep decline in exact-match accuracy as conflict size increases, consistent with the paper’s observation that DEEPMERGE performs best on small merge regions (≤3 lines).
...
> **Image description:** Line chart showing DEEPMERGE’s top‑1 precision (blue), recall (green), and F1 (red) as a function of the model confidence threshold (0.0–0.9). Precision rises steadily with higher thresholds (≈0.4 at 0.0 to ≈0.8+ by 0.9) while recall falls (≈0.36 at 0.0 down to ≈0.1 at 0.9); 95% confidence intervals are indicated by the dashed lines around each curve. The F1 score peaks around thresholds 0.4–0.5 at about 0.46, and at threshold 0.5 the authors report precision ≈0.72 and recall ≈0.34, illustrating the trade-off used to choose operating points that favor high precision with acceptable recall.
...
> **Image description:** This table compares DEEPMERGE’s top‑1 synthesis accuracy and the proportion of the dataset for two categorical classes of resolutions: CONCAT (resolutions that are a pure concatenation AB or BA) and OTHER (all other interleavings). CONCAT samples make up 26.88% of the dataset and achieve 44.40% top‑1 accuracy, while OTHER samples constitute 73.12% and achieve 29.03% top‑1 accuracy. In other words, DEEPMERGE is more accurate on CONCAT-style resolutions, although the majority of examples belong to the OTHER class where accuracy is lower.
