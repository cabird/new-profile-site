...
> **Image description:** Time-series line plot of modularity (range 0–1) for PostgreSQL developer email networks from ~1998 to ~2007, with a solid line for product-related messages and a dashed line for process-related messages. The product-topic curve is consistently higher (approximately 0.5–0.85) with peaks around 2001–2002 and again near 2005–2006; the process-topic curve is lower (roughly 0.25–0.5) but shows a rise toward ~0.6 near 2006–2007. Because modularity quantifies strength of community structure, the figure demonstrates that discussions that mention code artifacts (product topics) produce more strongly modular, tightly clustered subcommunities than process discussions over the examined period.
...
> **Image description:** A network snapshot (April–June 2003) of a single Python mailing-list subcommunity: diamond nodes denote developers, ovals denote other participants, and rectangular boxes denote top-level directories those people committed to. Black edges inside the boxed group show the dense email interactions among the participants, while the red curved edges link those same actors to many different directories (for example /Modules/, /Objects/, /Lib/, /Lib/test/, /Include/, /Doc/ext/, /Lib/distutils/), indicating commits spread across the code tree. The diagram visualizes the paper’s case study where this cohesive social group collaborated on garbage-collection diagnosis and fixes that cut across both C interpreter modules and Python library files, i.e., a cross-cutting technical concern rather than a single localized module.
